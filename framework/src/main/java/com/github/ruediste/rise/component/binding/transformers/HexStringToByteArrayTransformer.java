...
public class HexStringToByteArrayTransformer
        extends TwoWayBindingTransformer<byte[], String> {

    public HexStringToByteArrayTransformer() {
    }
    ...
    protected byte[] transformInvImpl(String source) {
        ...
        if ((source.length() % 2) != 0) {
            throw new ValidationException(new ValidationFailureImpl(
                    messges().lengthDoesNotMatch(source, source.length())));
        }
        source.codePoints().forEach(cp -> {
            ...
            throw new ValidationException(
                    new ValidationFailureImpl(messges().wrongCharacterFound(
                            source, new String(new int[] { cp }, 0, 1))));
        });
        String upperSource = source.toUpperCase();
        try {
            return BaseEncoding.base16().decode(upperSource);
        } catch (IllegalArgumentException e) {
            throw new ValidationException(new ValidationFailureImpl(messges()
                    .transformationFailed(upperSource, e.getMessage())));
        }
    }

    protected Messages messges() {
        return InjectorsHolder.getRestartableInjector()
                .getInstance(TMessageUtil.class)
