...
public class BoundComponentController extends ControllerComponent {
    ...
    static class View extends ViewComponent<BoundComponentController> {
        ...
        protected Component createComponents() {
            return new CPage(label(this)).render(toComponent(html -> html.render(toComponentBound(() -> controller.data(),
                    x -> x.span().ID("bound").content(controller.data().getValue())))

                    .render(toComponentDirect(x -> x.span().ID("direct").content(controller.data().getValue())))

                    .render(new CFormGroup(
                            new CTextField().CLASS("textField").bindText(() -> controller.data().getValue())))

                    .render(new CButton("pushDown").CLASS("pushDown").setHandler(() -> controller.pushDown()))
                    .render(new CButton("pullUp").CLASS("pullUp").setHandler(() -> controller.pullUp()))
