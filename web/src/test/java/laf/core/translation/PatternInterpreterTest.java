...
public class PatternInterpreterTest {
	...
	public void testSimple() {
		assertEquals("foo",
				format.format("foo", new HashMap<>(), Locale.ENGLISH));
	}
	...
	public void testSimpleParameter() {
		assertEquals("the 4",
				format.format("the {param}", map("param", 4), Locale.ENGLISH));
	}

	@Test
	public void testSimpleParameterEscape() {
		assertEquals("the 4", format.format("the {pa$}ram}", map("pa}ram", 4),
				Locale.ENGLISH));
	}
	...
	public void testParameterFormatType() {
		...
		assertEquals("the formatted", format.format("the {param, aFormat}",
	...
	public void testParameterFormatTypeStyle() {
		when(
				formatHandler.handle(eq(Locale.ENGLISH), eq(4), eq(" aSt$yle"),
						any(Function.class))).thenReturn("formatted");
		assertEquals("the formatted", format.format(
				"the {param, aFormat, aSt$yle}", map("param", 4),
				Locale.ENGLISH));
	}
	...
	public void testDate() {
		assertEquals(
				"the 2/1/14",
				format.format("the {param, date, short}",
